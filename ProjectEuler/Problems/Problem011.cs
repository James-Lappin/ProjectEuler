	...
	public class Problem11
	{
		...
		public int Solve()
		{
			...
			int[][] dave = new int[][]
			{
				...
				new [] {67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66, 49, 94, 21},
				new [] {24, 55, 58, 05, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72},
				new [] {21, 36, 23, 9, 75, 00, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34, 31, 33, 95},
				new [] {78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 04, 62, 16, 14, 9, 53, 56, 92},
				...
				new [] {04, 52, 8, 83, 97, 35, 99, 16, 07, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66},
				new [] {88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69},
				new [] {04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40, 62, 76, 36},
			...
			for (int i = 3; i < 20; i++)
			{
				for (int j = 0; j < 20; j++)
				{
					int sum = dave[i - 3][j] * dave[i - 2][j] * dave[i - 1][j] * dave[i][j];
			...
			for (int i = 0; i < 20; i++)
			{
				for (int j = 3; j < 20; j++)
				{
					int sum = dave[i][j - 3] * dave[i][j - 2] * dave[i][j - 1] * dave[i][j];
					if (sum > result) result = sum;
				}
			}
			...
			for (int i = 3; i < 20; i++)
			{
				for (int j = 3; j < 20; j++)
				{
					int sum = dave[i - 3][j - 3] * dave[i - 2][j - 2] * dave[i - 1][j - 1] * dave[i][j];
					if (sum > result) result = sum;
				}
			}
			...
			for (int i = 3; i < 20; i++)
			{
				for (int j = 3; j < 20; j++)
				{
					int sum = dave[i][j - 3] * dave[i - 1][j - 2] * dave[i - 2][j - 1] * dave[i - 3][j];
					if (sum > result) result = sum;
				}
			}
